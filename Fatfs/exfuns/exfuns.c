...
void test_fatfa_cvs()
{
    ...
    uint8_t BUF[25],p[25];																//BUF读写数据，p为文件路径
    uint8_t len=sizeof(BUF);
    if(exf_getfree((uint8_t *)"0:",&total,&free) == 0)									//得到SD卡的总容量和剩余容量
    {
        ...
        if(f_open (&fileobj,(const char *)p, FA_OPEN_ALWAYS | FA_READ | FA_WRITE) == FR_OK)
        {
            ...
            if(size == 0)																//第一次创建文件，需要添加标题栏
            {
                fr= f_lseek(&fileobj,size);
                {
                    ...
                    f_sync(&fileobj);													//刷新写文件的缓存信息
                }
            }																			//注：写完了标题栏不需要偏移指针了，因为前面写标题栏的时候加了换行，已经是第二行的首地址了
