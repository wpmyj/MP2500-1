...
//#define	ENTER_FF(fs) {ff_enter(fs);}//{taskENTER_CRITICAL();}//进入FATFS,关闭中断,防止相互干扰
//#define LEAVE_FF(fs, res) {ff_leave(fs);return res;}//{taskEXIT_CRITICAL();return res;}//离开FATFS,开启中断,任务继续执行
#endif
...
)
{
...
#endif
    {
        ...
        for (;;) {	/* Find a contiguous cluster block */
            ...
            if (clst == stcl) {
                res = FR_DENIED;    /* All cluster scanned? */
                break;
            }
        }
        if (res == FR_OK) {
