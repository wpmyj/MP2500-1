...
/**
 * @brief :	读取一个坐标值(x或者y),连续读取READ_TIMES次数据,对这些数据升序排列,然后去掉最低和最高LOST_VAL个数,取平均值
...
/**
 * @brief :	连续2次读取触摸屏IC,且这两次的偏差不能超过,ERR_RANGE,满足条件,则认为读数正确,否则读数错误.
 * @param :	x,y:读取到的坐标值
 * @retval:	0,失败;1,成功。
 */
...
{
    int tempfac;
    tempfac=BSP_AT24CXX_ReadOneByte(SAVE_ADDR_BASE+13);													//读取标记字,看是否校准过！
    if(tempfac==0X0A)																					//触摸屏已经校准过了
    {
        ...
        tp_dev.touchtype=BSP_AT24CXX_ReadOneByte(SAVE_ADDR_BASE+12);									//读取触屏类型标记
