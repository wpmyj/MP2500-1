...
#define SD_DMA_MODE     0	                        									//1：DMA模式，0：查询模式 (不适用DMA模式)
...
/**
 * @brief :	SSDMMC底层驱动，时钟使能，引脚配置，DMA配置（此函数会被HAL_SD_Init()调用）
 * @param :	hsd:SD卡句柄
 * @retval:	none
 */
...
/**
 * @brief :	通过DMA读取SD卡一个扇区
 * @param :
			buf:读数据缓存区
			sector:扇区地址
			blocksize:扇区大小(一般都是512字节)
			cnt:扇区个数
 * @retval:	返回值:错误状态;0,正常;其他,错误代码;
 */
uint8_t SD_ReadBlocks_DMA(uint32_t *buf,uint64_t sector,uint32_t blocksize,uint32_t cnt)
{
    uint8_t err=0;
...
void BSP_SdcardInit(void)
{
    ...
    printf("Card Capacity:%d MB\r\n",(uint32_t)(SDCardInfo.CardCapacity>>20));						//打印容量
    printf("Card BlockSize:%d\r\n\r\n",SDCardInfo.CardBlockSize);									//打印块大小
#endif
}
